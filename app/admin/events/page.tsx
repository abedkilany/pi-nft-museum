...
function badgeStyle(value: string | null | undefined) {
  ...
  if (value === 'SUCCESS') {
    return {
      background: 'rgba(89, 214, 131, 0.16)',
      borderColor: 'rgba(89, 214, 131, 0.4)',
    };
  }

  return {
    background: 'rgba(114, 132, 255, 0.14)',
    borderColor: 'rgba(114, 132, 255, 0.35)',
  };
}

function humanSummary(event: EventSummaryLike) {
  return (
    event.readableSummary ||
    event.message ||
    `${event.name} on ${event.route || event.feature || 'system'}`
  );
}
...
export default async function AdminEventsPage({
  ...
  const [events, groupedStatus, groupedCategory, latest, totalImportant] =
    await Promise.all([
      safeAppEventQuery(
        () =>
      ...
      safeAppEventQuery(
        () => prisma.appEvent.count({ where: toWhere({ preset: 'important' }) }),
        0
      ),
    ]);

  const statusSummary = Object.fromEntries(
    groupedStatus.map((row) => [row.status, row._count._all])
  );
    ...
    <div style={{ display: 'grid', gap: '24px' }}>
      <section className="card" style={{ padding: '24px' }}>
        <div className="section-head compact">
          ...
          <p>
            Focused event monitoring. This page now prioritizes warnings,
            failures, audit actions, and admin-impacting activity instead of
            every small click.
          </p>
        </div>
        ...
        <div className="card-actions" style={{ marginTop: '18px', flexWrap: 'wrap' }}>
          <a
            href="/admin/events?preset=important"
            className={`button ${preset === 'important' ? 'primary' : 'secondary'}`}
          >
            Important only
          </a>
          <a
            href="/admin/events?preset=errors"
            className={`button ${preset === 'errors' ? 'primary' : 'secondary'}`}
          >
            Errors only
          </a>
          <a
            href="/admin/events?preset=all"
            className={`button ${preset === 'all' ? 'primary' : 'secondary'}`}
          >
          ...
          {latest ? (
            <span className="pill">
              Last event {new Date(latest.createdAt).toLocaleString()}
            </span>
          ) : null}
        </div>
      </section>
      ...
      <section className="card" style={{ padding: '20px' }}>
        ...
        >
          ...
          <input
            name="q"
            placeholder="Search event, route, trace, entity"
      ...
      <section className="card" style={{ padding: '20px' }}>
        <div className="card-actions" style={{ flexWrap: 'wrap' }}>
          {groupedCategory.map((row) => (
            <span key={row.category} className="pill">
              {row.category}: {row._count._all}
            </span>
          ))}
        </div>
      </section>
              ...
              <tbody>
                {events.map((event) => (
                  <tr key={event.id}>
                    ...
                    >
                      <span className="pill" style={badgeStyle(event.status)}>
                        {event.status}
                      </span>
                      {event.severity ? (
                        <span
                    ...
                    >
                      <strong style={{ display: 'block' }}>
                        {event.route || event.feature || 'Unknown route'}
                      </strong>
                      <span style={{ color: 'var(--muted)' }}>
                    ...
                    >
                      {event.user ? (
                        <span>{event.user.username || event.user.email}</span>
                      ) : (
                        <span style={{ color: 'var(--muted)' }}>
                          Guest / system
                        </span>
