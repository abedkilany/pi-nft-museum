...
async function getReviewArtworks() {
...
export default async function ReviewPage() {
  const artworks = await getReviewArtworks();
    ...
    <div className="page-stack">
      ...
      <section className="card surface-section">
        ...
        <p style={{ margin: 0, color: 'var(--muted)' }}>
          Artworks are reviewed here before Lazy Mint. They do not appear in the
          main gallery until Lazy Mint succeeds.
        </p>
      </section>

      <section className="card surface-section">
        <p style={{ margin: 0 }}>
          Log in with Pi to rate artworks during review.
        </p>
      </section>
      ...
      {artworks.length === 0 ? (
        <section className="card surface-section">
          <p style={{ margin: 0 }}>
            No artworks are currently in public review.
          </p>
        </section>
      ) : (
            ...
            return (
              <article key={artwork.id} className="card split-list-card">
                ...
                <div className="split-list-side">
                  <AuthAwareRatingStars
                    artworkId={artwork.id}
                    currentAverage={Number(artwork.averageRating)}
                    currentVotes={artwork.ratingsCount}
                  />
                </div>
              </article>
            );
