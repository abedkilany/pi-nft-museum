...
function normalizeSection(section: AdminPageSectionInput, index: number) {
  return {
  ...
  try {
    ...
    const page = await prisma.page.create({
      data: {
        ...
        sections: {
          create: sections.map((section, index) => normalizeSection(section, index))
        }
      }
    });
    ...
    await createAuditLog({
      ...
      targetType: 'PAGE',
      targetId: page.id,
      newValues: { title, slug, status, sectionsCount: sections.length }
    });
