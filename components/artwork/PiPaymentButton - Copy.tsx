...
export function PiPaymentButton({ artworkId, title, amount, currency, disabled = false, disabledReason = null }: Props) {
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState<string | null>(null);

  const label = useMemo(() => `Pay ${amount.toFixed(2)} ${currency} on Testnet`, [amount, currency]);
  ...
  return (
    <div style={{ display: 'grid', gap: '10px' }}>
      <button className="button primary" type="button" onClick={handlePayment} disabled={disabled || loading}>
        {loading ? 'Opening Pi payment...' : label}
      </button>
      {disabledReason ? <p className="form-message">{disabledReason}</p> : null}
      {message ? <p className="form-message">{message}</p> : null}
      <p className="form-message" style={{ color: 'var(--muted)' }}>Test mode only. This flow is intended for Pi Testnet / Sandbox experiments.</p>
    </div>
  );
}
